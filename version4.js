...
function loadThisEvent(eventName) {
  ...
  // for ( let k in inflatedThing ) {
  //     const v = inflatedThing[k]
  //     console.log( "%c " + k + "   " + v , "background:pink")
  // }
...
//     const x = transformationModule.defaultCategorizedEvents
...
//         2
//     );
//      //MwaAnalytics.baseCollection(MwaAnalytics.EVENT_CATEGORY['ERROR'], MwaAnalytics.getMapOfCookies())
  ...
  try {
    ...
    for (let k in simple) {
      ...
      for (let k2 in flat_payload) {
        //if ( k2.endsWith(k)) {
        // Some keys change e.g., 'screentype' vs 'type' - I do not know when what changes so...  ...super permissive
